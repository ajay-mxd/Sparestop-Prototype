  ...
  return (
    <div className="space-y-6 h-auto md:h-[calc(100vh-100px)] flex flex-col pb-20 md:pb-0">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-2">
        ...
        <div className="text-sm text-primary bg-primary/10 px-3 py-1 rounded w-fit border border-primary/20">
          Wholesale Rate: 70% of MRP
        </div>
      </div>

      {successMsg && (
        <div className="bg-green-500/20 text-green-500 p-4 rounded-lg flex items-center border border-green-500/30">
      ...
      <div className="flex flex-col md:flex-row gap-6 flex-1 overflow-hidden">
        {/* Catalog */}
        <div className="flex-1 flex flex-col bg-surface rounded-xl shadow-sm border border-border overflow-hidden min-h-[400px]">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {filteredParts.map(part => (
              <div key={part.id} className="flex justify-between items-center p-3 border border-border rounded-lg hover:bg-background/50 transition-colors">
                <div>
        ...
        {/* Order Summary */}
        <div className="w-full md:w-80 bg-surface rounded-xl shadow-sm border border-border flex flex-col overflow-hidden min-h-[300px] md:min-h-0">
          <div className="p-4 bg-background/50 border-b border-border font-bold flex items-center gap-2 text-textPrimary">
              ...
              orderCart.map(item => (
                <div key={item.part.id} className="flex justify-between items-center text-sm">
                  <div className="flex-1">
                    <div className="font-medium text-textPrimary truncate w-32">{item.part.name}</div>
                    <div className="text-xs text-textSecondary">₹{(item.part.price * 0.7).toFixed(0)} x {item.quantity}</div>
                  </div>
                  <div className="flex items-center gap-2 text-textPrimary">
                    <button onClick={() => updateQuantity(item.part.id, -1)} className="p-1 hover:bg-background rounded"><Minus size={14} /></button>
                    <span className="w-4 text-center">{item.quantity}</span>
                    <button onClick={() => updateQuantity(item.part.id, 1)} className="p-1 hover:bg-background rounded"><Plus size={14} /></button>
                  </div>
                </div>
              ))
            )}
          ...
          <div className="p-4 border-t border-border bg-background/50">
            <div className="flex justify-between items-center mb-4">
              ...
              <span className="font-bold text-lg text-textPrimary">₹{totalEstimate.toFixed(0)}</span>
            </div>
            <button 
              onClick={handlePlaceOrder}
              disabled={orderCart.length === 0 || isSubmitting}
              className="w-full bg-primary text-white py-3 rounded-lg font-bold hover:bg-blue-600 disabled:opacity-50 transition-colors"
            >
