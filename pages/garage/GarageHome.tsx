  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="bg-primary text-white p-6 rounded-2xl shadow-lg">
        ...
        <Link to="/garage/search" className="bg-white text-textPrimary p-3 rounded-lg flex items-center w-full shadow-sm">
      ...
      <div>
        ...
        <div className="grid grid-cols-4 gap-4">
          {partCategories.map(cat => {
            ...
            return (
              <Link to={`/garage/search?cat=${cat.id}`} key={cat.id} className="flex flex-col items-center">
                <div className="w-14 h-14 bg-white rounded-xl shadow-sm flex items-center justify-center mb-2 border border-border">
                  <IconComponent size={24} className="text-primary" />
                </div>
                <span className="text-xs text-center text-textSecondary font-medium">{cat.name}</span>
              </Link>
            );
          })}
        </div>
      ...
      <div>
        ...
        <div className="space-y-4">
          {[1, 2, 3].map(i => (
            <div key={i} className="bg-white p-4 rounded-xl border border-border flex gap-4">
               <div className="w-20 h-20 bg-gray-100 rounded-lg flex-shrink-0"></div>
               <div className="flex-1">
                 <h3 className="font-semibold">Brake Pads Front</h3>
                 ...
                 <div className="flex justify-between items-center">
                   ...
                   <button className="text-xs bg-blue-50 text-primary px-3 py-1 rounded-full font-medium">View</button>
                 </div>
               </div>
            </div>
          ))}
